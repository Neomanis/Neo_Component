...
test("should show correct text", async ({ mount }) => {
    const component = await mount(<UneditableField variant="primary" label="UneditableField" data="et hop" />);
...
test("should get variants css with secondary bg", async ({ mount }) => {
    ...
    await expect(component.locator('[data-testid="uneditablefield-data"]')).not.toHaveCSS(
        "color",
        "rgb(127, 239, 127)"
    );
});
