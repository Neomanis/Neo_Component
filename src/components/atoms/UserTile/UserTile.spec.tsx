...
test("should display add user variant", async ({ mount }) => {
    let clicked = false;
    const component = await mount(<UserTile type="user" onSelect={() => (clicked = true)} showName />);
...
test("should display user variant", async ({ mount }) => {
    const component = await mount(<UserTile type="user" user={fakeUser} onSelect={() => {}} showName />);
...
test("should display add group variant", async ({ mount }) => {
    let clicked = false;
    const component = await mount(<UserTile type="group" onSelect={() => (clicked = true)} showName />);
    await expect(component.locator("div").first()).toHaveClass(/hover:bg-neo-blue/);
...
test("should display group variant", async ({ mount }) => {
    const component = await mount(<UserTile type="group" group={fakeGroups[0]} onSelect={() => {}} showName />);
